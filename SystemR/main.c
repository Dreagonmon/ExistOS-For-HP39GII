...
static void render_datetime() {
    char *text = pvPortMalloc(20); // "yyyy-mm-dd\nHH:MM:SS\0"
...
static void render_content() {
    ...
    // Date Time: 48 pixel
    render_datetime();
    // charge status: last 8 pixel
    if (ll_get_charge_status()) {
        ui_text_area(
            font8x8_quan, ui_trs(TEXTG_CHARGING1), get_frame_buffer(),
            ui_CONTENT_X, ui_CONTENT_Y + ui_CONTENT_H - font8x8_quan->char_height, ui_CONTENT_W, font8x8_quan->char_height,
            ui_ALIGN_HCENTER | ui_ALIGN_VCENTER,
            COLOR_SET, COLOR_CLEAR
        );
    }
